      ...
      <div style={{ borderTop: '1px solid #334155', paddingTop: 8, display: 'flex', flexDirection: 'column', gap: 5, flex: 1 }}>
        {prods.map((p) => (
          <div key={p.id} style={{ display: 'flex', gap: 8, alignItems: 'flex-start' }}>
            <span style={{ fontSize: 15, fontWeight: 800, color: '#64748b', lineHeight: 1.4, minWidth: 26, flexShrink: 0 }}>
              {p.cantidad}×
            </span>
            <div>
...
export default function KDS() {
  ...
  const fetchOrder = useCallback(async (id) => {
    const { data } = await supabase
      .from('pedidos')
      .select(`
        id, empresa, estado, total, notas, forma_pago, created_at,
        productos_pedido (
          ...
          <>
            ...
            {proximos.length > 0 && (
              <div>
                ...
                <div style={{
                  display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)',
                  gap: 12, alignItems: 'start', opacity: 0.45, filter: 'saturate(0.3)',
                }}>
                  {proximos.map((pedido) => (
                    <PedidoCard
                      key={pedido.id}
